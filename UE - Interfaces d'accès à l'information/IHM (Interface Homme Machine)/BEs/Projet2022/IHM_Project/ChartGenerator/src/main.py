...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    ...
    assert isinstance(agent_object, ChartGenerator)
...
# inputs
def coin1_input_callback(iop_type, name, value_type, value, my_data):
    agent_object = my_data
    assert isinstance(agent_object, ChartGenerator)
    agent_object.coin1I = value
    pass
    # add code here if needed


def coin2_input_callback(iop_type, name, value_type, value, my_data):
    agent_object = my_data
    assert isinstance(agent_object, ChartGenerator)
    agent_object.coin2I = value
    pass

def start_time_input_callback(iop_type, name, value_type, value, my_data):
    agent_object = my_data
    assert isinstance(agent_object, ChartGenerator)
    agent_object.start_timeI = value
    pass

def end_time_input_callback(iop_type, name, value_type, value, my_data):
    agent_object = my_data
    assert isinstance(agent_object, ChartGenerator)
    agent_object.end_timeI = value
    pass
    # add code here if needed


def generate_input_callback(iop_type, name, value_type, value, my_data):
    agent_object = my_data
    assert isinstance(agent_object, ChartGenerator)
    agent_object.generate_chart()
...
if __name__ == "__main__":
    ...
    igs.input_create("coin1", igs.STRING_T, None)
    igs.input_create("coin2", igs.STRING_T, None)
    igs.input_create("start_time", igs.INTEGER_T, None)
    igs.input_create("end_time", igs.INTEGER_T, None)
    igs.input_create("generate", igs.IMPULSION_T, None)

    igs.output_create("chart1_uri", igs.STRING_T, None)
    igs.output_create("chart2_uri", igs.STRING_T, None)
    igs.output_create("error_msg", igs.STRING_T, None)

    igs.observe_input("coin1", coin1_input_callback, agent)
    igs.observe_input("coin2", coin2_input_callback, agent)
    igs.observe_input("start_time", start_time_input_callback, agent)
    igs.observe_input("end_time", end_time_input_callback, agent)
    igs.observe_input("generate", generate_input_callback, agent)
