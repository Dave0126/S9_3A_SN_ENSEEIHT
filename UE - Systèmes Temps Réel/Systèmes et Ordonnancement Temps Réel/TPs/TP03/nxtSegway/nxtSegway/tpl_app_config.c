...
/* Resource mutex */
#define mutex_id 0
CONST(ResourceType, AUTOMATIC) mutex = mutex_id;

/*=============================================================================
 * Declaration of processes IDs
 */

/* Task backwards identifier */
#define backwards_id 0
CONST(TaskType, AUTOMATIC) backwards = backwards_id;

/* Task display identifier */
#define display_id 1
CONST(TaskType, AUTOMATIC) display = display_id;

/* Task pendulum identifier */
#define pendulum_id 2
CONST(TaskType, AUTOMATIC) pendulum = pendulum_id;
...
/* Alarm backwards_alarm identifier */
#define backwards_alarm_id 0
CONST(AlarmType, AUTOMATIC) backwards_alarm = backwards_alarm_id;

/* Alarm display_alarm identifier */
#define display_alarm_id 1
CONST(AlarmType, AUTOMATIC) display_alarm = display_alarm_id;

/* Alarm pendulum_alarm identifier */
#define pendulum_alarm_id 2
CONST(AlarmType, AUTOMATIC) pendulum_alarm = pendulum_alarm_id;
 ...
 * pendulum
 */
VAR(tpl_resource, OS_VAR) mutex_rez_desc = {
 ...
 */
/*-----------------------------------------------------------------------------
 * Task backwards descriptor
 */
#define APP_Task_backwards_START_SEC_CODE
#include "tpl_memmap.h"
/*
 * Task backwards function prototype
 */
FUNC(void, OS_APPL_CODE) backwards_function(void);
#define APP_Task_backwards_STOP_SEC_CODE
#include "tpl_memmap.h"
 ...
 */
/*
 * Task backwards stack
 */
#define APP_Task_backwards_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) backwards_stack_zone[512/sizeof(tpl_stack_word)];
#define APP_Task_backwards_STOP_SEC_STACK
#include "tpl_memmap.h"

#define backwards_STACK {backwards_stack_zone, 512}

/*
 * Task backwards context
 */
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_context, OS_VAR) backwards_int_context;

#define backwards_CONTEXT &backwards_int_context
...
/*
 * Resources used by task backwards
 *
 * mutex
 */

/*
 * Static descriptor of task backwards
 */
CONST(tpl_proc_static, OS_CONST) backwards_task_stat_desc = {
  /* context                  */  backwards_CONTEXT,
  /* stack                    */  backwards_STACK,
  /* entry point (function)   */  backwards_function,
  /* internal ressource       */  NULL,
  /* task id                  */  backwards_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
#endif
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task backwards
 */
VAR(tpl_proc, OS_VAR) backwards_task_desc = {
  /* resources                      */  NULL,
...
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) display_stack_zone[512/sizeof(tpl_stack_word)];
#define APP_Task_display_STOP_SEC_STACK
#include "tpl_memmap.h"

#define display_STACK {display_stack_zone, 512}
...
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_context, OS_VAR) display_int_context;

#define display_CONTEXT &display_int_context
...
/*-----------------------------------------------------------------------------
 * Task pendulum descriptor
 */
#define APP_Task_pendulum_START_SEC_CODE
#include "tpl_memmap.h"
/*
 * Task pendulum function prototype
 */
FUNC(void, OS_APPL_CODE) pendulum_function(void);
#define APP_Task_pendulum_STOP_SEC_CODE
#include "tpl_memmap.h"
 ...
 * Task pendulum stack
 */
#define APP_Task_pendulum_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) pendulum_stack_zone[512/sizeof(tpl_stack_word)];
#define APP_Task_pendulum_STOP_SEC_STACK
#include "tpl_memmap.h"

#define pendulum_STACK {pendulum_stack_zone, 512}

/*
 * Task pendulum context
 */
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_context, OS_VAR) pendulum_int_context;

#define pendulum_CONTEXT &pendulum_int_context
...
/*
 * Resources used by task pendulum
 *
 * mutex
 */

/*
 * Static descriptor of task pendulum
 */
CONST(tpl_proc_static, OS_CONST) pendulum_task_stat_desc = {
  /* context                  */  pendulum_CONTEXT,
  /* stack                    */  pendulum_STACK,
  /* entry point (function)   */  pendulum_function,
  /* internal ressource       */  NULL,
  /* task id                  */  pendulum_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task pendulum
 */
VAR(tpl_proc, OS_VAR) pendulum_task_desc = {
  /* resources                      */  NULL,
#if WITH_MEMORY_PROTECTION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_MEMORY_PROTECTION */
...
tpl_stat_proc_table[TASK_COUNT+ISR_COUNT+1] = {
  &backwards_task_stat_desc,
  &display_task_stat_desc,
  &pendulum_task_stat_desc,
 ...
 */
/*-----------------------------------------------------------------------------
 * Alarm backwards_alarm descriptor
 *
 * This alarm does the activation of task backwards
 */
...
CONST(tpl_task_activation_action, OS_CONST) backwards_alarm_action = {
  {
...
CONST(tpl_alarm_static, OS_CONST) backwards_alarm_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_TRACE == YES)
    /* id of the alarm for tracing  */  , backwards_alarm_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&backwards_alarm_action
};
...
VAR(tpl_time_obj, OS_VAR) backwards_alarm_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&backwards_alarm_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  40,
    /* date                         */  1,
...
/*-----------------------------------------------------------------------------
 * Alarm display_alarm descriptor
 *
...
CONST(tpl_task_activation_action, OS_CONST) display_alarm_action = {
  {
    /* action function  */  tpl_action_activate_task
  },
  /* task id            */  display_id
...
CONST(tpl_alarm_static, OS_CONST) display_alarm_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_TRACE == YES)
    /* id of the alarm for tracing  */  , display_alarm_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&display_alarm_action
};
...
VAR(tpl_time_obj, OS_VAR) display_alarm_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&display_alarm_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  500,
    /* date                         */  1,
...
/*-----------------------------------------------------------------------------
 * Alarm pendulum_alarm descriptor
 *
 * This alarm does the activation of task pendulum
 */
...
CONST(tpl_task_activation_action, OS_CONST) pendulum_alarm_action = {
  {
    /* action function  */  tpl_action_activate_task
  },
  /* task id            */  pendulum_id
...
CONST(tpl_alarm_static, OS_CONST) pendulum_alarm_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_TRACE == YES)
    /* id of the alarm for tracing  */  , pendulum_alarm_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&pendulum_alarm_action
};
...
VAR(tpl_time_obj, OS_VAR) pendulum_alarm_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&pendulum_alarm_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  4,
    /* date                         */  1,
...
CONSTP2VAR(tpl_time_obj, AUTOMATIC, OS_APPL_DATA)
  ...
  &backwards_alarm_alarm_desc,
  &display_alarm_alarm_desc,
  &pendulum_alarm_alarm_desc
};
#define OS_STOP_SEC_CONST_UNSPECIFIED
#include "tpl_memmap.h"
...
VAR(tpl_proc_id, OS_VAR) tpl_priority_2_fifo[4];
